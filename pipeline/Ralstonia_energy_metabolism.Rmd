...
One part of the data used here (growth on **fructose**, **succinate**, **formate**) was already published in a previous research article with focus on *C. necator*'s central carbon metabolism. The [preprint is available on BioRxiv](https://www.biorxiv.org/content/10.1101/2021.03.21.436304v2). The second part (autotrophic growth on **hydrogen**, anoxic growth with **nitrate respiration**) is new data that complements previous results. The focus of this pipeline is 
...
df_fitness_h2 <- fitness_gene %>% ungroup %>%
  filter(condition %in% c("NO3", "H2")) %>%
  mutate(substrate = recode(condition, NO3 = "nitrate", H2 = "hydrogen"),
    condition = if_else(substrate == "nitrate", "continuous", "pulse"))

# merge fitness tables
df_fitness <- bind_rows(df_fitness_frc, df_fitness_suc, df_fitness_h2) %>%
  rename(locus_tag = locusId)
rm("df_fitness_frc", "df_fitness_suc", "df_fitness_h2", "fitness_gene")
...
custom_splom(df_fitness_comp %>%
  select(matches("conti|pulse")), pch = 19, cex = 0.3, col = grey(0.4, 0.4))
```

```{r, include = FALSE}
df_test_no3 <- read_tsv("/home/michael/Documents/SciLifeLab/Resources/rebar/data/20210624_barseq_mixed/results/result.poolcount") %>%
  select(barcode, matches("NO3_")) %>%
  pivot_longer(cols = matches("NO3_"), names_to = "condition", values_to = "count") %>%
  separate(condition, sep = "_", into = c("condition", "time", "sample", "replicate")) %>%
  group_by(barcode, time) %>%
  summarize(count = mean(count)) %>%
  pivot_wider(names_from = "time", values_from = "count")

df_fitness_summary %>% ungroup %>%
  filter(substrate == "hydrogen", condition == "pulse") %>%
  select(locus_tag, norm_gene_fitness_median, time) %>%
  pivot_wider(names_from = "time", values_from = "norm_gene_fitness_median")
  
  xyplot(log10(`8gen`) ~ log10(`0gen`), df_test_no3,
    par.settings = custom.colorblind(),
    pch = 19, alpha = 0.1, cex = 0.7, aspect = 1,
...
## Unsupervised clustering of genes/mutants by fitness
...
mat_heatmap <- mat_heatmap[order.dendrogram(as.dendrogram(mat_cluster)), c(1,5,2,6,4,3,7)]
...
heatmap_fitness <- function(data, key = TRUE) {
  ...
  data %>%
  #group_by(gene_name) %>% arrange(cond_short) %>%
...
### Substrate assimilation: hydrogenases & Co
...
plot_moco_fit <- df_fitness_summary %>% filter(time == 8, grepl("mo[aedbg]", gene_name)) %>%
...
```{r, fig.width = 7, fig.height = 3}
plot_fdh_fit <- df_fitness_summary %>% filter(time == 8, grepl("fd[swho]", gene_name)) %>%
...
- **hydrogenase**
  - hoxA = master regulator, 2-component system. HoxA is probably phosphorylated by a 
    sensory component (hypX) and then acts in conjunction with sigma-54 as a transcriptional activator.
  ...
  - HoxJ =  RH binding signal transducer (?), histidine protein kinase
    ...
    CO, thereby providing the carbonyl ligand for hydrogenases,
...
```{r, fig.width = 7, fig.height = 3}
plot_hyd_fit <- df_fitness_summary %>% filter(time == 8, grepl("hox", gene_name)) %>%
  mutate(gene_name = stri_extract_first_regex(gene_name, "hox[A-Z0-9]+")) %>%
  ungroup %>% mutate(gene_name = factor(gene_name, paste0("hox", c("A","K","G","Z","M",
    "L","O","Q","R","T","V","N","N2","B","C","J",
    "F","U","Y","H","W","I")))) %>%
  heatmap_fitness
...
- **hydrogenase pleiotropy genes (maturation)**
  - hypA1,B1,F1,C1,D1,E1 = metallocenter formation proteins for both MBH and SH. "HypC1, hypD1, 
  ...
  - hypX = CO cofactor formation for NiFe-hydrogenase. HypX converts formyl-THF into water and CO
  - hypA2,B2,F2 = alternative genes for hypA1,B1,F1, not sufficient alone for Hyd formation
  - hypF3,hypC2,hypD2,hypE2,hypA3,hypB3.   The   physiologicalfunction of these extrahypgenes is unclear. 

```{r, fig.width = 7, fig.height = 3}
plot_hyp_fit <- df_fitness_summary %>% filter(time == 8, grepl("hyp", gene_name)) %>%
...
<!-- - **electron transport chain** -->
<!--   - cyoA1B1C1D1, ... (3 copies) = bo3 complex, quinole oxidase, low O2 affinity -->
<!--   - cydA1B1, ... (2 copies) = bd complex, quinole oxidase, high O2 affinity? -->
