...
## Interesting clusters: Hydrogen metabolism genes

**Potential CO2 transporters or carbonic anhydrases**

Different gene clusters appear based on the fitness pattern in different conditions. Clusters specific for litho-autotrophic metabolism are particularly interesting. We can add the cluster IDs to the main data frame, and investigate fitness.
...
df_fitness_summary %>%
  ...
  # plot heatmap
  levelplot(norm_gene_fitness_median ~ gene_name * cond_short, .,
    par.settings = custom.colorblind(), #colorkey = key,
    col.regions = colorRampPalette(heat_cols)(12),
...
df_co2_transport_align %>%
  ...
  # plot heatmap
  levelplot(norm_gene_fitness_median ~ 
      factor(paste(target_name, format(rank, digits = 2), "-", locus_tag)) * cond_short, .,
    par.settings = custom.colorblind(), #colorkey = key,
    col.regions = colorRampPalette(heat_cols)(12),
    at = seq(-6, 6, 1), aspect = "iso",
    xlab = "", ylab = "", scales = list(cex = 0.7, x = list(rot = 90)),
...
df_moco <- df_fitness_summary %>% filter(time == 8, grepl("mo[aedbg]", gene_name)) %>%
  mutate(gene_name = str_remove(gene_name, "^moaA ")) %>%
...
The RBA modeling result is very clear: A increase in the protein pool (= decrease of unutilized protein mass) does *not* result in the observed changes of growth rate of up to +50%. The change in growth rate is rather proportional to the amount of saved protein cost.
