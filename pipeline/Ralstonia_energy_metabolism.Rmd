...
plot_cost_vs_mu <- xyplot(mean_mass_fraction_norm*100 ~ mean_mutant_mu,
    ...
    panel = function(x, y, pointlabels, ...) {
      panel.grid(h = -1, v = -1, col = grey(0.9))
      panel.superpose(x, y, ...)
      panel.repellabels(x, y, labels = pointlabels, y_boundary = c(0.5,1.5),
...
plot_cost_vs_mu2 <- xyplot(corrected_cost ~ mean_mutant_mu,
    df_mutant_cost, groups = substrate,
    pointlabels = as.character(df_mutant_cost$gene_name),
    par.settings = custom.colorblind(), col = stdcol[2:4],
    xlab = expression("µ [h"^-1*"]"),
    ...
    panel = function(x, y, pointlabels, ...) {
      panel.grid(h = -1, v = -1, col = grey(0.9))
      panel.superpose(x, y, ...)
      panel.repellabels(x, y, labels = pointlabels, y_boundary = c(1,4.5),
        draw_box = TRUE, cex = 0.6, ...)},
    panel.groups = function(x, y, ...) {
      panel.xyplot(x, y, ...)
      panel.lmlineq(x, y, r.squared = TRUE, label = "", ...)
...
print(plot_cost_vs_mu, position = c(0,0,0.51,0.59), more = TRUE)
print(plot_cost_vs_mu2, position = c(0.5,0,1,0.59), more = TRUE)
```
...
## Prediction of growth advantage using a resource balance analysis (RBA) model
