...
Different figures are made to visualize:
- growth rate (similar to above)
- protein abundance as relative mass fraction (protein cost)
- protein abundance that is saved for each particular KO
- and finally, correlation of protein cost and growth rate advantage of KO mutants

With the last item it is important to consider that a KO by transposon integration will have an effect on the downstream gene expression, effectively disrupting transcription/translation of the entire operon downstream of the integration site.
...
If we assume that each KO knocks out the gene expression of all consecutive genes of the operon, we need to sum up protein cost for these effected gene sets; we make a binary matrix of associations for each KO with a number of affected genes (0 = FALSE, 1 = TRUE), and multiply with protein mass.
...
rownames(mat_mutant_genomic) <- filter(df_mutant_cost, substrate == "formate") %>% pull(gene_name) %>% as.character()
df_mutant_cost <- df_mutant_cost %>% group_by(substrate) %>%
  mutate(
    corrected_cost = mean_mass_fraction_norm %>% {colSums(.*100*mat_mutant_genomic)},
    corrected_sd = sd_massfraction %>% {colSums(.*100*mat_mutant_genomic)}
  )
```
...
plot_mutant_cost <- xyplot(mean_mass_fraction_norm*100 ~ gene_name, df_mutant_cost,
    ...
    lwd = 2, col = stdcol[2:4], ylim = c(-0.2, 5.0), pch = 1,
...
print(plot_mutant_mu, position = c(-0.02, 0.62, 1, 1), more = TRUE)
print(plot_mutant_cost, position = c(0.02, 0.31, 1, 0.68), more = TRUE)
print(plot_cost_vs_mu2, position = c(-0.02, 0, 0.5, 0.38))
```
