...
df_fitness_lowco2 <- fitness_gene %>% ungroup %>%
  mutate(
    substrate = if_else(condition == "NO3_batch", "nitrate", "hydrogen"),
    condition = if_else(condition == "NO3_batch", "pulse", "continuous"))
...
mat_heatmap <- mat_heatmap[order.dendrogram(as.dendrogram(mat_cluster)), c(4,9,3,8,1,6,5,10,2,7)]
```
...
Extract cluster number for each gene and perform GO or KEGG enrichment per cluster.
The goal is to identify the most prevalent biological functions per cluster.
...
plot_keggenrich <- df_kegg_enrichment %>%
  ...
  group_by(cluster) %>%
  arrange(desc(log10_p_value)) %>% slice(1:5) %>% ungroup %>%
  mutate(kegg_pathway = str_remove(kegg_pathway, "metabolism|biosynthesis")) %>%
  
  xyplot(factor(cluster, rev(unique(cluster))) ~ log10_p_value, .,
    groups = cluster, labels = .[["kegg_pathway"]],
    par.settings = custom.colorblind(), col = custom_colors,
    xlab = expression("-log"[10]*" p-value"), ylab = "cluster",
...
```{r, echo = FALSE}
svg(filename = "../figures/figure_nmds_tsne.svg", width = 8, height = 8)
print(plot_nmds, split = c(1,1,2,2), more = TRUE)
print(plot_tsne, split = c(2,1,2,2), more = TRUE)
print(plot_keggenrich, split = c(1,2,1,2), more = TRUE)
dev.off()
```
...
- **terminal cytochrome C oxidases**
  - coxMNOPQ = bb3 complex, affinity unknown, not essential for growth or N-fixation in B.
    japonycum where it was discovered. []()
...
print(plot_etc_fit, position = c(0,0.20,1,1.05), more = TRUE)
...
Different gene clusters appear based on the fitness pattern in different conditions. Clusters specific for lithoautotrophic metabolism are particularly interesting. We can add the cluster IDs to the main data frame, and investigate fitness.
...
```{r, fig.width = 8, fig.height = 4.2}
df_fitness_summary %>%
  filter(cluster != 1, time == 8, !str_detect(gene_name, "hox|hyp")) %>%
  group_by(gene_name) %>% filter(all(norm_gene_fitness_median[c(3,8)] < -2) | locus_tag %in% c("H16_A0325", "H16_A0326")) %>%
...
Investigating interaction networks with STRING database only reveals the obvious connections of **pts** and **hem** enzymes. However there are some more interesting candidates:

- **ptsI + ptsH (+ glnE)** - according to databases, a clear hit for a PEP-dependent sugar PTS system. This can't be true for _C. necator_ otherwise it would not show the familiar pattern of depletion on hydrogen and growth benefit on other substrates. This PTS system must be implicated in hydrogen metabolism, probably a transporter? The genes downstream of ptsIH do not seem to be affected (H16_A0327, 8, 9).
- **hemxD + hemY** - genes involved in heme biosynthesis; hemXD (hemD) = Uroporphyrinogen III synthase, hemY = uncharacterized function. Other hem genes such as hemC are either essential, or KO shows no particular phenotype (hemP, hemK1, hemN). Heme is an important cofactor for enzymes of the ETC. Importantly, heme is not a part of hydrogenases excpet for the cytochrome b-like subunit hoxZ from MBH.
...
<!-- Other genes that were essential on only one autotrophic condition (high CO2): -->
...
<!-- - **H16_A2742** - Putative AMP nucleosidase (AMP ), no known domains or published functions. 315 AA, 9 mutants in library. Surrounding genes neutral to growth. -->
...
## Comparison of genes important for low vs high CO2 autotrophy

- two conditions with lithoautotrophic growth are available
- they differ in some points such as bioreactor vs sealed flask, date and time of cultivation etc
- main difference is gas composition: pulsed (flask) cultivation has 15% CO2
- continuous (bioreactor) cultivation has athmosperic CO2 (0.05%)

```{r, fig.width = 6, fig.height = 6}
df_fitness_summary %>%
  filter(substrate == "hydrogen", time == 8, !is.na(gene_name)) %>%
  select(gene_name, condition, norm_gene_fitness_median) %>%
  pivot_wider(names_from = condition, values_from = norm_gene_fitness_median) %>%
  mutate(
    dfit = pulse - continuous,
  ...
  xyplot(continuous ~ pulse, data = ., groups = significant,
    ...
    par.settings = custom.colorblind(), col = custom_colors,
    xlim = c(-9, 3), ylim = c(-9, 3),
    xlab = "H2 + high CO2 (P)", ylab = "H2 + low CO2 (C)",
    panel = function(x, y, labels, ...) {
      panel.grid(h = -1, col = grey(0.9))
      panel.abline(a = 0, b = 1, col = grey(0.5), lty = 2, size = 0.8)
      panel.abline(a = 4, b = 1, col = grey(0.5), lty = 2, size = 0.8)
...
All CAs with quantified fitness are neutral during lithoautotrophic growth (H16_A1192 - _cag_, H16_B2403 - _caa_, H16_B2270 - _can2_). Only H16_A0169 (_can_) is strictly essential, no mutants are available. This is different to the results from [Gai et al., AMB Express, 2014](https://link.springer.com/article/10.1186/2191-0855-4-2) that found two essential CAs, _can_ and _caa_: 

"All four purified CAs were capable of performing the interconversion of CO2 and HCO3– [...] Deletion of _can_ affected the growth of R. eutropha; however the growth defect could be compensated by adding CO2 to the culture. Deletion of _caa_, encoding an alpha-CA, had the strongest deleterious influence on cell growth.". 

Also [Kusian et al., J Bac, 2002](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC135314/) report the essentiality of _can_ for lithoautotrophic growth. _Caa_ fitness was quantified in the library with 11 mutants, yet does not show any phenotype on any substrate. The results in Gai et al. are somewhat contradictory, as the _caa_ completion test in a quadruple-CA mutant had no beneficial effect on growth. 

Altogether, no specific bicarbonate transporters are annotated in _C. necator_. However, important bicarbonate (HCO3+) or CO2 transporters known from e.g. cyanobacteria (_Synechocystis_ sp. PCC 6803) could be used to search for homologous genes in _C. necator_. The following CO2/HCO3+ transporters are known in _Synechocystis_:
...
- **NDH** (nuo operon/): all genes of this operon are essential in _C. necator_, no cond. fitness

The first three of these genes were blasted against the genome of _C. necator_. The figure shows alignment scores for the top 20 hits.
...
```
BicA and cmpB seem to have some very similar homologs in the _C. necator_ genome. However, none of these homologs has a clear phenotype related to hydrogen metabolism.
...
list_hyd_selection <- c(paste0("hox", c("A", "K", "G", "Z", "F", "U", "Y", "H")),
    paste0("hyp", c("A", "B", "C", "D", "E", "F", "X")))
  
df_increased_mu <- df_fitness_summary %>%
  filter(
    grepl("hox[KGZFUYHA]|hyp[BCDEFX]1? |hypA PHG012", gene_name), condition == "continuous",
    substrate %in% c("formate", "fructose", "succinate")) %>%
  mutate(gene_name = stri_extract_first_regex(gene_name, "hox[KGZFUYHA]|hyp[ABCDEFX]")) %>%
  mutate(gene_name = factor(gene_name, list_hyd_selection))

plot_increased_mu <- xyplot(norm_gene_fitness_median ~ time | gene_name, df_increased_mu,
    groups = substrate, as.table = TRUE, layout = c(5,3),
...
plot_mutant_mu <- xyplot(mutant_mu ~ gene_name, df_mutant_mu,
    ...
    col = stdcol[2:4], xlab = "", ylab = expression("µ [h"^-1*"]"),
...
print(plot_increased_mu, position = c(0.035,0.35,1,1), more = TRUE)
print(plot_mutant_mu, position = c(0,0,1,0.43))
```

```{r, echo = FALSE, results = 'hide'}
svg("../figures/figure_mutant_mu.svg", width = 6.5, height = 6.5)
print(plot_increased_mu, position = c(0.035,0.35,1,1), more = TRUE)
print(plot_mutant_mu, position = c(0,0,1,0.43))
dev.off()
```
...
The first plot is the protein abundance as relative mass fraction (protein cost), the second is the correlation of protein cost and growth rate advantage of KO mutants. Here it is important to consider that a KO by transposon integration will have an effect on the downstream gene expression, effectively disrupting transcription/translation of the entire operon downstream of the integration site. This is shown in the third plot with estimated total protein cost saved by KO.
...
df_mutant_cost <- df_mutant_mu %>%
  ...
  inner_join(by = c("locus_tag", "substrate"),
    Ralstonia_eutropha %>% ungroup %>% filter(growthrate == 0.1) %>%
      select(locus_tag, substrate, mean_mass_fraction_norm, sd_massfraction)) %>%
  arrange(gene_name)
```

If we assume that each KO knocks out the gene expression of all consecutive genes of the operon, we need to sum up protein cost these effected gene sets; make a binary matrix of associations for each KO with a number of affected genes (0 = FALSE, 1 = TRUE), and multiply with protein mass.

```{r}
mat_mutant_genomic <- matrix(ncol = length(list_hyd_selection),
  data = c(rep(1, 15), rep(c(0,1,0), c(1,3,11)),
    rep(c(0,1,0), c(2,2,11)), rep(c(0,1,0), c(3,1,11)),
    rep(c(0,1,0), c(4,4,7)), rep(c(0,1,0), c(5,3,7)),
    rep(c(0,1,0), c(6,2,7)), rep(c(0,1,0), c(7,1,7)),
    rep(c(0,1), c(8,7)), rep(c(0,1), c(9,6)),
    rep(c(0,1), c(10,5)), rep(c(0,1), c(11,4)),
    rep(c(0,1), c(12,3)), rep(c(0,1), c(13,2)),
    rep(c(0,1), c(14,1))
  )
)

df_mutant_cost <- df_mutant_cost %>% group_by(substrate) %>%
  mutate(corrected_cost = mean_mass_fraction_norm %>% {colSums(.*100*mat_mutant_genomic)})
```

All plots together on one canvas.

```{r, fig.width = 6.5, fig.height = 5.7}
plot_mutant_cost <- xyplot(mean_mass_fraction_norm*100 ~ gene_name, df_mutant_cost,
    ...
    lwd = 2, col = stdcol[2:4], ylim = c(-0.05, 1.55),
    xlab = "", ylab = "protein mass fraction [%]",
    panel = function(x, y, ...) {
      panel.grid(h = -1, v = -1, col = grey(0.9))
      panel.barplot(x, y, beside = TRUE, ...)
      panel.key(pch = 15, corner = c(0.98, 0.95), ...)
    }
  )

plot_cost_vs_mu <- xyplot(mean_mutant_mu ~ mean_mass_fraction_norm*100,
    ...
    xlab = "protein mass fraction [%]",
    ylab = expression("µ [h"^-1*"]"),
    panel = function(x, y, pointlabels, ...) {
      panel.grid(h = -1, v = -1, col = grey(0.9))
      panel.superpose(x, y, ...)
      panel.repellabels(x, y, labels = pointlabels, x_boundary = c(0.5,1.5),
        draw_box = TRUE, cex = 0.6, ...)},
...
plot_cost_vs_mu2 <- xyplot(mean_mutant_mu ~ corrected_cost,
    df_mutant_cost, groups = substrate,
    pointlabels = as.character(df_mutant_cost$gene_name),
    par.settings = custom.colorblind(), col = stdcol[2:4],
    xlab = "protein mass fraction [%]",
    ...
    panel = function(x, y, pointlabels, ...) {
      panel.grid(h = -1, v = -1, col = grey(0.9))
      panel.superpose(x, y, ...)
      panel.repellabels(x, y, labels = pointlabels, x_boundary = c(1,4.5),
        draw_box = TRUE, cex = 0.6, ...)},
    panel.groups = function(x, y, ...) {
      panel.xyplot(x, y, ...)
      panel.abline(h = 0.1, col = grey(0.5), lty = 3, lwd = 2)
...
print(plot_mutant_cost, position = c(0,0.5,1,1), more = TRUE)
print(plot_cost_vs_mu, position = c(0,0,0.53,0.59), more = TRUE)
print(plot_cost_vs_mu2, position = c(0.47,0,1,0.59))
```


```{r, echo = FALSE, results = 'hide'}
svg("../figures/figure_mutant_cost.svg", width = 6.5, height = 5.7)
print(plot_mutant_cost, position = c(0,0.5,1,1), more = TRUE)
print(plot_cost_vs_mu, position = c(0,0,0.53,0.59), more = TRUE)
print(plot_cost_vs_mu2, position = c(0.47,0,1,0.59))
dev.off()
...
The RBA modeling result is very clear: An increase in the protein pool (= decrease of unutilized protein mass) does *not* result in the observed changes of growth rate of up to +50%. The change in growth rate is rather proportional to the amount of saved protein cost.
...
plot_cost_predict <- df_macr %>%
  ...
  xyplot(value ~ reduced_protein_cost, .,
    ...
    ylim = c(0.09, 0.155), col = stdcol[2:4],
    xlab = "increase protein pool size [%]",
    ylab = expression("µ [h"^-1*"]"),
    panel = function(x, y, z, ...) {
      panel.grid(h = -1, v = -1, col = grey(0.9))
      panel.xyplot(x, y, ...)
...
plot_cost_predict <- plot_cost_predict + as.layer(
  xyplot(mean_mutant_mu ~ corrected_cost,
    df_mutant_cost %>%
      filter(gene_name %in% c("hoxA", "hoxF", "hoxU", "hoxY", "hoxH")),
    groups = substrate, col = stdcol[2:4],
    pointlabels = as.character(df_mutant_cost$gene_name),
    panel = function(x, y, pointlabels, ...) {
      panel.superpose(x, y, ...)
      panel.repellabels(x, y, labels = pointlabels, y_boundary = c(0.11,0.16),
        draw_box = TRUE, cex = 0.6, ...)},
    panel.groups = function(x, y, ...) {
      panel.xyplot(x, y, ...)
      panel.lmline(x, y, ...)
...
```{r, echo = FALSE, results = 'hide'}
svg("../figures/figure_cost_predict.svg", width = 3.7, height = 3.7)
print(plot_cost_predict)
dev.off()
```
